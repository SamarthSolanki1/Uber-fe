...
const ConfirmRidepopup = (props) => {
  ...
  const submithandler = async (e) => {
    ...
    try {
      const response = await axios.get(`${import.meta.env.VITE_BASE_URL}/rides/start-ride`, {
        params: {
          rideId: props.ride?._id,  // Sending as query params
          otp: otp.trim() // Trim spaces to avoid issues
        },
        headers: {
  ...
  return (
    <div>
    <h5 onClick={() => {props.setridePopUppanel(false)}} className='p-1 text-center absolute w-[90%]'><i className="text-3xl text-black ri-arrow-down-wide-line"></i></h5>
      <h3 className='text-2xl font-semibold mb-5'>Start Your Ride</h3>
      <div className='flex items-center justify-between mt-3 p-3 rounded-lg bg-gray-100'>
        ...
        <h5 className='text-lf font-semibold'>2.2 km away</h5>
      </div>
  ...
  <input 
    ...
    className='bg-[#eee] px-6 py-4 rounded-lg font-mono text-base w-full mt-3 mb-7' 
    type='text' 
    placeholder='Enter OTP' 
  />
  <button type="submit" className='w-full bg-green-400 text-black font-semibold p-2 rounded-lgme rounded-2xl'>
    Confirm
  </button>
  <button type="button" onClick={() => {
      props.setConfirmRidePopupPanel(false);
      props.setRidePopupPanel(false);
    }} className='w-full mt-3 bg-red-500 text-black font-semibold p-2 rounded-lgme rounded-2xl'>
    Cancel
  </button>
</form>
   </div>
